package org.apache.hadoop.yarn.mpi.client;
...
public class Client {
  ...
  // Is the MPI application running, we do NOT use 'volatile' for passing
  // reference
  ...
  public void reloadConfiguration() {
    ...
    clientTimeout = conf.getLong(MPIConfiguration.MPI_TIMEOUT,
        24 * 60 * 60 * 1000);
    jvmOptions = conf.get(
        MPIConfiguration.MPI_APPLICATION_JAVA_OPTS_EXCEPT_MEMORY, "");
  }
  ...
  public boolean init(String[] args) throws ParseException, IOException {
    ...
    opts.addOption("M", "master-memory", true,
    ...
    opts.addOption("q", "queue", true,
        "RM Queue in which this application is to be submitted");
    opts.addOption(
        "t",
        "timeout",
        true,
        "Wall-time, Application timeout in milliseconds, default is 86400000ms, i.e. 24hours");
    opts.addOption("n", "num-containers", true,
        "No. of containers on which the mpi program needs to be executed");
    ...
    opts.addOption(
        ...
        "processes-per-container",
        true,
        "processes per container,take effect when all containers download the same file");
    ...
    Option downSame = OptionBuilder.withArgName("property=value").hasArgs(2)
        .withValueSeparator()
    ...
    Option output = OptionBuilder.withArgName("property=value").hasArgs(2)
        .withValueSeparator()
    ...
    if (cliParser.hasOption("master-memory")) {
      amMemory = Integer.parseInt(cliParser.getOptionValue("master-memory",
          "64"));
    }
    if (amMemory <= 0) {
    ...
    if (!cliParser.hasOption("mpi-application")) {
      throw new IllegalArgumentException(
          "No mpi executable program specified, exiting.");
    }
    ...
    if (cliParser.hasOption("container-priority")) {
      containerPriority = Integer.parseInt(cliParser.getOptionValue(
    ...
    if (cliParser.hasOption("processes-per-container")) {
      ppc = Integer.parseInt(cliParser.getOptionValue(
  ...
  private void parseInput(CommandLine cliParser) {
    ...
    Enumeration<String> sameFileNames = (Enumeration<String>) sameFiles
        .propertyNames();
    ...
    while (true) {
      ...
      if (mpiClient == null && isRunning.get()) {
        LOG.info("Got application report from ASM for" + ", appId="
            ...
            + ", appMasterHost=" + report.getHost() + ", rpcPort:"
            + report.getRpcPort() + ", appQueue=" + report.getQueue()
      ...
      int logInterval = conf.getInt(MPIConfiguration.MPI_LOG_PULL_INTERVAL,
          1000);
  ...
  private static ApplicationId parseAppId(String appIdStr) {
    ...
    if (!m.matches())
      return null;
    ApplicationId appId = ApplicationId.newInstance(Long.parseLong(m.group(1)),
